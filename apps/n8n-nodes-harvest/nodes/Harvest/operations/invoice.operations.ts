...
export function isBinaryResponse(value: unknown): value is BinaryResponse {
	return (
		typeof value === 'object' && value !== null && '__binary' in value && (value as BinaryResponse).__binary === true
	)
}
...
export async function executeInvoiceOperation(
	...
	if (operation === 'create') {
		...
		const additionalFields = context.getNodeParameter('invoiceAdditionalFields', i) as InvoiceAdditionalFields
		...
		if (additionalFields.paymentOptions && additionalFields.paymentOptions.length > 0) {
			invoiceBody.payment_options = additionalFields.paymentOptions as CreateInvoice['payment_options']
		}
	...
	if (operation === 'downloadPdf') {
		...
		if (!clientKey) {
			throw new Error('Client Key is required. You can retrieve it from the invoice using the "Get" operation.')
		}
		...
		const buffer = Buffer.from(new Uint8Array(pdfBuffer as ArrayBuffer)) as Buffer
